...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    context = context.transpose(1,2).reshape((scores.shape[0],-1,self.num_attention_heads * self.attention_head_size))
    return context
...
class BertLayer(nn.Module):
  def __init__(self, config):
    ...
    self.attention_layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps) #769
    self.attention_dropout = nn.Dropout(config.hidden_dropout_prob)
    # feed forward
    self.interm_dense = nn.Linear(config.hidden_size, config.intermediate_size) #768 * 3k
    self.interm_af = F.gelu
    # layer out
    self.out_dense = nn.Linear(config.intermediate_size, config.hidden_size) #3k * 768
    self.out_layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps) #768
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    """
    normalized_output = ln_layer(dropout(dense_layer(output)) + input)
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # add-norm layer
    # feed forward
    feedforward_output = self.interm_dense(self_attention_output)
    feedforward_output = self.interm_af(feedforward_output)
    # another add-norm layer
    feedforward_output = self.add_norm(self_attention_output, feedforward_output, self.out_dense , self.out_dropout, self.out_layer_norm)
